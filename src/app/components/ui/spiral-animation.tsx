...
class AnimationController {
  ...
  public cameraZ = -400;
  private readonly baseCameraZ = -400;
  public cameraTravelDistance = 3400;
  private readonly startDotYOffset = 28;
  public viewZoom = 100;
  private readonly baseViewZoom = 100;
  private readonly numberOfStars = 5000;
  private readonly trailLength = 80;
  public scrollProgress = 0;
  public slowRotation = 0;
  ...
  private createStars() {
    for (let i = 0; i < this.numberOfStars; i++) {
      this.stars.push(new Star(this.baseCameraZ, this.cameraTravelDistance));
    }
  }
  ...
  public setScrollProgress(progress: number) {
    ...
    this.cameraZ = this.baseCameraZ + progress * 2000;
    this.viewZoom = this.baseViewZoom + progress * 75;
    this.render();
  }
  ...
  public showProjectedDot(position: Vector3D, sizeFactor: number) {
    ...
    const dynamicCameraZ =
      this.cameraZ +
      this.ease(Math.pow(t2, 1.2), 1.8) * this.cameraTravelDistance;

    if (position.z > dynamicCameraZ) {
      const dotDepthFromCamera = position.z - dynamicCameraZ;
  ...
  public render() {
    ...
    ctx.rotate(
      -Math.PI * this.ease(t2, 2.7) +
        this.slowRotation +
        this.scrollProgress * 1.8,
    );
    this.drawTrail(t1);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (animationRef.current) {
        animationRef.current.setScrollProgress(progress);
      }
    };
